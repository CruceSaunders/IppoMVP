...
struct LootBoxOpeningSheet: View {
    ...
    private let totalItems = 100 // Items in the spin strip (enough for smooth spinning)
    private let visibleItems = 5 // Items visible at once
    private let animationDuration: Double = 5.5 // Total spin duration in seconds
    ...
    private func generateSpinItems() {
        ...
        // Place the winning item at position 85-90 (near end but with buffer)
        // This creates the classic CS:GO effect where you "almost" get better items
        // Position 85-90 out of 100 gives ~9,180-9,720px scroll distance
        winningIndex = 85 + Int.random(in: 0...5)
    ...
    private func startSpin() {
        phase = .spinning
        scrollOffset = 0 // Ensure we start at 0
        ...
        // Use a timer for smooth, controlled animation (60fps)
        ...
        animationTimer = Timer.scheduledTimer(withTimeInterval: 1.0/60.0, repeats: true) { timer in
            ...
            // Custom easing: stays fast longer, then dramatic slowdown for suspense
            // Phase 1 (0-75%): Almost linear, fast spinning
            // Phase 2 (75-100%): Sharp deceleration for suspense
            let easedProgress: Double
            if progress < 0.75 {
                // Fast phase: slight ease-in to get up to speed, then mostly linear
                let fastProgress = progress / 0.75
                easedProgress = 0.85 * (0.3 * fastProgress + 0.7 * pow(fastProgress, 0.7))
            } else {
                // Suspense phase: dramatic slowdown in final 25%
                let slowProgress = (progress - 0.75) / 0.25
                // Quintic ease-out for dramatic deceleration
                let slowEased = 1.0 - pow(1.0 - slowProgress, 5.0)
                easedProgress = 0.85 + 0.15 * slowEased
            }
            
            // Update scroll position
            scrollOffset = targetOffset * CGFloat(easedProgress)
            
            // Update phase based on progress - "slowing" starts at 75% when suspense kicks in
            if progress >= 0.75 && phase == .spinning {
                phase = .slowing
            }
            
            // Play tick haptics based on scroll position change
            let currentItemIndex = Int(scrollOffset / (itemWidth + itemSpacing))
            let previousItemIndex = Int((scrollOffset - targetOffset * CGFloat(1.0/60.0 / animationDuration)) / (itemWidth + itemSpacing))
            if currentItemIndex != previousItemIndex && (phase == .spinning || phase == .slowing) {
