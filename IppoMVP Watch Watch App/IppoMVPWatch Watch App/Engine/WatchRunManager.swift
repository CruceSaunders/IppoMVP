...
final class WatchRunManager: NSObject, ObservableObject {
    ...
    override init() {
        super.init()
        // Request HealthKit permissions asynchronously, don't block init
        Task {
    ...
    private func requestHealthKitPermissions() async {
        // Check if HealthKit is available (it may not be on Simulator)
    ...
    func startRun() {
        ...
        print("🏃 RUN: Warmup period \(encounterConfig.warmupDuration)s, then encounter checks begin")
        DispatchQueue.main.asyncAfter(deadline: .now() + encounterConfig.warmupDuration) { [weak self] in
            print("🏃 RUN: Warmup complete, starting encounter checks")
    ...
    func endRun() {
        ...
        print("   Duration: \(Int(elapsedTime))s (\(minutes) min)")
        print("   Sprints: \(sprintsCompleted)/\(totalSprints) completed")
        print("   RP: \(earnedRP) (passive: \(passiveRP))")
        print("   XP: \(earnedXP) (passive: \(passiveXP))")
        print("   Coins: \(earnedCoins)")
        print("   LootBoxes: \(earnedLootBoxes)")
        print("   Pet caught: \(caughtPetId ?? "none")")
    ...
    private func startWorkoutSession() {
        // Skip workout session if HealthKit isn't available (e.g., Simulator)
        guard HKHealthStore.isHealthDataAvailable() else {
            print("⚠️ Skipping workout session - HealthKit not available")
            return
        }
    ...
    private func checkForEncounter() {
        guard runState == .running && !isInRecovery && !isPaused else { 
            return 
        }
        ...
        let isPityTimer = timeSinceLast >= encounterConfig.pityTimerMax
        print("⚔️ ENCOUNTER CHECK: timeSinceLast=\(String(format: "%.1f", timeSinceLast))s, probability=\(String(format: "%.2f", probability)), roll=\(String(format: "%.3f", roll)), pity=\(isPityTimer)")
        
        if roll < probability || isPityTimer {
            print("⚔️ ENCOUNTER: Sprint triggered! (roll < prob: \(roll < probability), pity: \(isPityTimer))")
            triggerSprint()
        }
    }
    ...
    private func triggerSprint() {
        ...
        print("🏃‍♂️ SPRINT #\(totalSprints): TRIGGERED!")
        print("   Duration: \(String(format: "%.1f", targetSprintDuration))s")
        print("   Baseline HR: \(baselineHR)")
    ...
    private func completeSprint() {
        ...
        print("🏃‍♂️ SPRINT #\(totalSprints): COMPLETE")
        print("   Peak HR: \(peakHR), Peak Cadence: \(peakCadence)")
        ...
        if isValid {
            sprintsCompleted += 1
            print("✅ SPRINT VALIDATED - Success! (\(sprintsCompleted)/\(totalSprints))")
            ...
            print("🎁 SPRINT REWARDS: +\(rp) RP, +\(xp) XP, +\(coins) coins")
            
            // Roll for loot box
            let lootRoll = Double.random(in: 0...1)
            if lootRoll < 0.70 {
                let rarity = rollLootBoxRarity()
                earnedLootBoxes.append(rarity)
                print("📦 LOOT BOX: \(rarity) (roll: \(String(format: "%.2f", lootRoll)))")
            } else {
                print("📦 LOOT BOX: No drop (roll: \(String(format: "%.2f", lootRoll)))")
            }
            ...
            print("❌ SPRINT FAILED - Validation did not pass")
        ...
        runState = .running
        print("🏃 RUN: Back to running state, encounter checks resuming")
        startEncounterChecks()
    }
    
    private func validateSprint() -> Bool {
        guard !sprintHRSamples.isEmpty else { 
            print("⚠️ VALIDATION: No HR samples, auto-fail")
            return false 
        }
        ...
        // Cadence Score
        let avgCadence = sprintCadenceSamples.reduce(0, +) / max(1, sprintCadenceSamples.count)
        ...
        print("📊 VALIDATION:")
        print("   HR: baseline=\(baselineHR), peak=\(peakHR), increase=\(hrIncrease), score=\(String(format: "%.2f", hrScore))")
        print("   Cadence: avg=\(avgCadence), peak=\(peakCadence), score=\(String(format: "%.2f", cadenceScore))")
        print("   Total Score: \(String(format: "%.1f", totalScore))% (threshold: 60%)")
    ...
    private func checkForPetCatch() {
        ...
        let catchRoll = Double.random(in: 0...1)
        print("🐾 PET CHECK: roll=\(String(format: "%.3f", catchRoll)), rate=\(catchRate)")
        ...
        if catchRoll < catchRate {
            ...
            if let petId = caughtPetId {
                print("🎉 PET CAUGHT: \(petId)!")
                print("   Bonus: +100 RP, +200 XP, +500 coins")
            ...
            print("🐾 PET CHECK: No catch this time")
        }
    }
    ...
    private func startRecovery() {
        ...
        print("⏸️ RECOVERY: Started \(sprintConfig.recoveryDuration)s recovery period")
    }
    
    // MARK: - Debug Methods
    func debugTriggerSprint() {
        print("🔧 DEBUG: Manually triggering sprint")
        guard runState == .running else {
            print("🔧 DEBUG: Cannot trigger sprint - not in running state (current: \(runState))")
            return
        }
        isInRecovery = false  // Skip recovery for testing
        triggerSprint()
    }
    
    func debugCompleteSprintSuccess() {
        print("🔧 DEBUG: Force completing sprint as SUCCESS")
        guard runState == .sprinting else {
            print("🔧 DEBUG: Cannot complete sprint - not in sprinting state")
            return
        }
        sprintTimer?.invalidate()
        
        // Force valid values
        peakHR = baselineHR + 30  // Ensure HR increase passes
        peakCadence = 180  // Ensure cadence passes
        sprintHRSamples = [peakHR]  // Need at least one sample
        sprintCadenceSamples = [peakCadence]
        
        completeSprint()
    }
    
    func debugCompleteSprintFail() {
        print("🔧 DEBUG: Force completing sprint as FAIL")
        guard runState == .sprinting else {
            print("🔧 DEBUG: Cannot complete sprint - not in sprinting state")
            return
        }
        ...
        peakHR = baselineHR  // No HR increase
        peakCadence = 50  // Low cadence
        sprintHRSamples = [peakHR]
        sprintCadenceSamples = [peakCadence]
        
        completeSprint()
    }
    
    func debugTriggerPetCatch() {
        print("🔧 DEBUG: Forcing pet catch")
        let petIds = ["pet_01", "pet_02", "pet_03", "pet_04", "pet_05",
                     "pet_06", "pet_07", "pet_08", "pet_09", "pet_10"]
        caughtPetId = petIds.randomElement()
        
        if let petId = caughtPetId {
            print("🎉 DEBUG PET CAUGHT: \(petId)!")
            WatchHapticsManager.shared.playPetCatch()
            earnedRP += 100
            earnedXP += 200
            earnedCoins += 500
    ...
    func debugTestAllHaptics() {
        print("🔧 DEBUG: Testing all haptics...")
        Task {
            print("🔧 Testing: playRunStart")
            WatchHapticsManager.shared.playRunStart()
            try? await Task.sleep(nanoseconds: 1_000_000_000)
            
            print("🔧 Testing: playSprintStart")
            WatchHapticsManager.shared.playSprintStart()
            try? await Task.sleep(nanoseconds: 1_000_000_000)
            
            print("🔧 Testing: playTick")
            WatchHapticsManager.shared.playTick()
            try? await Task.sleep(nanoseconds: 500_000_000)
            
            print("🔧 Testing: playSprintSuccess")
            WatchHapticsManager.shared.playSprintSuccess()
            try? await Task.sleep(nanoseconds: 1_000_000_000)
            
            print("🔧 Testing: playSprintFail")
            WatchHapticsManager.shared.playSprintFail()
            try? await Task.sleep(nanoseconds: 1_000_000_000)
            
            print("🔧 Testing: playSprintEnd")
            WatchHapticsManager.shared.playSprintEnd()
            try? await Task.sleep(nanoseconds: 1_000_000_000)
            
            print("🔧 Testing: playPetCatch")
            WatchHapticsManager.shared.playPetCatch()
            try? await Task.sleep(nanoseconds: 1_500_000_000)
            
            print("🔧 Testing: playRunEnd")
            WatchHapticsManager.shared.playRunEnd()
            
            print("🔧 DEBUG: Haptic test complete!")
        }
    }
    
    func debugSkipRecovery() {
        print("🔧 DEBUG: Skipping recovery period")
        isInRecovery = false
        recoveryRemaining = 0
    }
}
